...
def logistic_regression_model(observations, input_data):
    w = pyro.sample('w', dist.Laplace(torch.zeros(input_data.shape[1]), 0.01 * torch.ones(input_data.shape[1])).to_event(1))
...
def boosting_bbvi():
    ...
    log_likelihood = 0
    for i in range(n_samples):
        predictive_trace = trace(block(wrapped_predictive_model, expose=['obs'])).get_trace()
